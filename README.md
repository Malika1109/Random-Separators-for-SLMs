...
- **`separator_accuracy_distribution_[model_name]/`**  
  - Summary statistics for experiments are saved as `.csv` files, e.g:
    ...
    - `optimization_mode`  
    - `dataset`  
    - `average_train_accuracy` — average accuracy of 160 separators on the training set  
    - `baseline_accuracy` — accuracy of the baseline on the training set  
    - `percent_better_than_baseline` — % of separators that performed better than the baseline in the training set  
    - `num_better_than_baseline` — number of separators (out of 160) better than the baseline in the training set  
    - `total_separators` - total number of separators in the experiment (160 for reproducability)
    - `average_test_accuracy` — average score of the top 5 separators on the test set  
    - `baseline_test_accuracy` — score of the baseline on the test set  
  
    For Generative tasks, summary statistics include: 
    - `seed`
    - `optimisation mode`
    - `dataset`
    - `total separators` 
    - `average training scores (of all 160) and test scores (of top 5) separators`
